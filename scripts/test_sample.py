...
def plot_sampling(s0, k0, y, s_samples, k_samples, y_noiseless, y_hat, 
                  model_name, config, sigma_y):
    # Extract images to plot and send everything to numpy
    s0 = s0.reshape(5, config.dataset.res, config.dataset.res).cpu().numpy()
    k0 = k0.reshape(5, config.dataset.res, config.dataset.res).cpu().numpy()
    y = y.reshape(5, config.dataset.res, config.dataset.res).cpu().numpy()
    s_samples = s_samples.reshape(5, 5, config.dataset.res, config.dataset.res).cpu().numpy()
    k_samples = k_samples.reshape(5, 5, config.dataset.res, config.dataset.res).cpu().numpy()
    y_noiseless = y_noiseless.reshape(5, config.dataset.res, config.dataset.res).cpu().numpy()
    y_hat = y_hat.reshape(5, 5, config.dataset.res, config.dataset.res).cpu().numpy()
    ...
    for i in range(5):
        ...
        # saving the plot
        os.makedirs('./results/test_sample', exist_ok=True)
        save_dir = f'./results/test_sample/{model_name}_sample{i}.pdf'
        plt.tight_layout()
...
def main(config):
    ...
    y_hat = lensingmodel.simulate_lensing(s_samples.reshape(25,1,config.dataset.res,config.dataset.res), 
                                          rim.rim_to_caustics(k_samples).reshape(25,1,config.dataset.res,config.dataset.res), 
                                          noise=False).reshape(5,5,1,config.dataset.res,config.dataset.res)
...
if __name__ == "__main__":
    if len(sys.argv) != 2:
        print("Usage: python test_sample.py <config_path>")
        sys.exit(1)
