...
def plot_sampling(s0, k0, y, s_samples, k_samples, y_noiseless, y_hat, 
    ...
    for i in range(num_images):
        ...
        # saving the plot
        os.makedirs('./results/test_ood_source', exist_ok=True)
        save_dir = f'./results/test_ood_source/{model_name}_sample{i}.pdf'
        plt.tight_layout()
...
def main(config):
    ...
    # Get test examples (kappa only) from the loader
    _, k0 = next(iter(test_loader))
...
if __name__ == "__main__":
    if len(sys.argv) != 2:
        print("Usage: python test_ood_source.py <config_path>")
        sys.exit(1)
