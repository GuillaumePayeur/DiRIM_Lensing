...
def save_h5(s, k, y, y_noiseless, s_samples, k_samples, y_samples, model_name):
    with h5py.File(f'./results/test_ood_kappa/{model_name}.h5', 'w') as f:
...
def plot_sampling(s0, k0, y, s_samples, k_samples, y_noiseless, y_hat, 
    ...
    for i in range(num_images):
        ...
        # saving the plot
        os.makedirs('./results/test_ood_kappa', exist_ok=True)
        save_dir = f'./results/test_ood_kappa/{model_name}_sample{i}.pdf'
        plt.tight_layout()
...
if __name__ == "__main__":
    if len(sys.argv) != 2:
        print("Usage: python test_ood_kappa.py <config_path>")
        sys.exit(1)
